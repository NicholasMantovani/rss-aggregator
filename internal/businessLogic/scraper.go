...
func scrapeFeed(db *database.Queries, wg *sync.WaitGroup, feed database.Feed) {
	...
	for _, item := range rssFeed.Channel.Item {
		...
		if err != nil {
			log.Printf("Clound parse date %v with err %v", item.PubDate, err)
			continue
		}

		_, err = db.CreatePost(context.Background(), database.CreatePostParams{ID: uuid.New(), CreatedAt: time.Now(), UpdatedAt: time.Now(),
			Title: item.Title, Description: description, PublishedAt: pubDate, Url: item.Link, FeedID: feed.ID})
		if err != nil {
